...
$PAGE->set_url('/moodle/plagiarism/sst/plagiarism_sst_report.php', [
    'cmid' => $cmid,
    'userid' => $userid,
...
if (empty($moduleenabled) || empty($modulesettings['plagiarism_sst_enable'])) {
    echo html_writer::div(get_string('disabledformodule', 'plagiarism_sst'), null, ['style' => $errormessagestyle]);
} else {
    ...
    if (!$isinstructor && empty($modulesettings['plagiarism_sst_allowstudentaccess'])) {
        echo html_writer::div(get_string('nopageaccess', 'plagiarism_sst'), null, ['style' => $errormessagestyle]);
    } else {
        echo html_writer::tag(
            'iframe',
            null,
            [
                'title' => get_string('report_title', 'plagiarism_sst'),
        ...
        if ($viewmode == 'course') {
            echo html_writer::link(
                ...
                ['title' => get_string('openfullscreen', 'plagiarism_sst')]
            );
        }
    }
}
